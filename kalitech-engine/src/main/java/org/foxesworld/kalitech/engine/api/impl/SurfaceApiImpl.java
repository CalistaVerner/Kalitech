 ...
 *
 * Contract:
 * - Only SurfaceHandle is accepted by API. No raw ids, no coercion, no reflection.
 * - Surface ids are still numeric (runtime registry), but never accepted as "Object".
 * - Entity binding is UUID-only (string) in public API.
 * - Dense entityId exists internally (ECS), never exposed.
 */
public final class SurfaceApiImpl extends AbstractApiModule implements SurfaceApi {
    ...
    public SurfaceApiImpl() {
        super("surface", "Surface", "3.0.0"); // NO legacy
    }
    ...
    private static void applyMaterialRecursiveWithTileWorld(Spatial s, Material mat, Value cfgOrNull) {
        if (s instanceof Geometry g) {
            g.setMaterial(mat);
            if (cfgOrNull != null) applyTileWorldToGeometryIfAny(g, cfgOrNull);
            return;
        }
        if (s instanceof TerrainQuad tq) {
            tq.setMaterial(mat);
    ...
    private static void applyUvScaleNonAccumulating(Geometry g, float u, float v) {
        ...
        // meshes are often shared: clone once per-geometry
    ...
    private static double num(Value v, String k, double def) {
        ...
        if (!m.isNumber()) return def;
        return m.asDouble();
    }

    // ------------------------------------------------------------
    // Core SurfaceApi exports (NO legacy overloads)
    // ------------------------------------------------------------
    ...
    public void attach(ApiContext ctx) {
        ...
        // bind UUID registry (ECS bridge used by SurfaceRegistry internally)
        this.registry.attach(ctx);
    ...
    public void setMaterial(SurfaceHandle target, Object materialHandleOrCfg) {
        ...
        onJmeSyncVoid("setMaterial", () -> {
            ...
            if (s instanceof Geometry g) {
                g.setMaterial(mat);
                if (cfg != null) applyTileWorldToGeometryIfAny(g, cfg);
    ...
    public void setTransform(SurfaceHandle target, Value cfg) {
        Objects.requireNonNull(target, "target");
        onJmeSyncVoid("setTransform", () -> applyTransform(requireSpatial(target), cfg));
    }
    ...
    public void setPos(SurfaceHandle target, Object pos) {
        Objects.requireNonNull(target, "target");
        onJmeSyncVoid("setPos", () -> requireSpatial(target).setLocalTranslation(vec3Any(pos, 0f, 0f, 0f)));
    }
    ...
    public void setCull(SurfaceHandle target, String hint) {
        Objects.requireNonNull(target, "target");
        onJmeSyncVoid("setCull", () -> requireSpatial(target).setCullHint(parseCullHint(hint)));
    }
    ...
    public void setVisible(SurfaceHandle target, boolean visible) {
        Objects.requireNonNull(target, "target");
        onJmeSyncVoid("setVisible", () -> requireSpatial(target)
    ...
    public void setScale(SurfaceHandle target, Object scale) {
        ...
        onJmeSyncVoid("setScale", () -> {
            ...
            s.setLocalScale(vec3Any(scale, 1f, 1f, 1f));
        });
    }
    ...
    public void setShadowMode(SurfaceHandle target, String mode) {
        Objects.requireNonNull(target, "target");
        onJmeSyncVoid("setShadowMode", () -> requireSpatial(target).setShadowMode(parseShadowMode(mode)));
    }
    ...
    public void destroy(SurfaceHandle target) {
        ...
        onJmeSyncVoid("destroy", () -> {
            // 1) physics cleanup (if module supports it)
            PhysicsApi p = this.physicsApi;
            if (p instanceof PhysicsApiImpl impl) impl.__cleanupSurface(target.id());

            // 2) detach from scene graph (if attached)
            try {
                registry.detachFromParent(target.id());
            } catch (Throwable ignored) {
            }

            // 3) detach entity binding (UUID-only) + remove ECS component
            String uuid = "";
            try {
                uuid = registry.detachSurface(target.id());
            } catch (Throwable ignored) {
            }
