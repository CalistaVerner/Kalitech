...
public final class LuaCfg {
    private LuaCfg() {
    }

    // ---------- Basic ----------

    public static boolean has(LuaValueRef v) {
        return v != null && !v.isNull();
    }

    public static LuaValueRef member(LuaValueRef v, String key) {
        return (has(v) && v.hasMember(key)) ? v.getMember(key) : null;
    }

    public static String str(LuaValueRef cfg, String key, String def) {
        LuaValueRef m = member(cfg, key);
    ...
    public static boolean bool(LuaValueRef cfg, String key, boolean def) {
        LuaValueRef m = member(cfg, key);
        if (m == null || m.isNull()) return def;
        try {
            return m.asBoolean();
    ...
    public static double num(LuaValueRef cfg, String key, double def) {
        LuaValueRef m = member(cfg, key);
        if (m == null || m.isNull()) return def;
        try {
            return m.asDouble();
    ...
    public static int intR(LuaValueRef cfg, String key, int def) {
        return (int) Math.round(num(cfg, key, def));
    }

    public static int intClampR(LuaValueRef cfg, String key, int def, int lo, int hi) {
        return clamp((int) Math.round(num(cfg, key, def)), lo, hi);
    }

    public static double numClamp(LuaValueRef cfg, String key, double def, double lo, double hi) {
    ...
    public static Vector2f vec2(LuaValueRef v, Vector2f def) {
        if (!has(v)) return def;
        try {
    ...
    public static int i32(LuaValueRef cfg, String key, int def) {
        if (cfg == null || cfg.isNull() || !cfg.hasMember(key)) return def;
        LuaValueRef v = cfg.getMember(key);
    ...
    public static double f64(LuaValueRef cfg, String key, double def) {
        if (cfg == null || cfg.isNull() || !cfg.hasMember(key)) return def;
        LuaValueRef v = cfg.getMember(key);
        if (v == null || v.isNull()) return def;
        try {
    ...
    public static boolean has(LuaValueRef v, String k) {
    ...
    public static ColorRGBA color(LuaValueRef cfg, String key, ColorRGBA def) {
        LuaValueRef v = member(cfg, key);
    ...
    /**
     * Reads a Lua table or a bridged host numeric array into {@code float[]}.
     *
     * @throws IllegalArgumentException if the value is not an array
     */
    public static float[] readFloatArray(LuaValueRef v) {
        if (v == null || v.isNull()) {
        ...
        // 2) Lua table or bridged host array
        if (v.hasArrayElements()) {
            ...
            for (int i = 0; i < len; i++) {
                LuaValueRef e = v.getArrayElement(i);
                if (e == null || e.isNull()) {
                    out[i] = 0f;
                    continue;
                }
                ...
                out[i] = 0f; // non-numeric Lua values become zero
            }
