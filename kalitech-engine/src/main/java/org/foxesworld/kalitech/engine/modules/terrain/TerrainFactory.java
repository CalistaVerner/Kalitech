...
public final class TerrainFactory {
    ...
    public TerrainQuad createTerrainFromHeights(Value cfg) {
        ...
        // read heights robustly (host arrays / typed arrays / polyglot arrays)
        float[] heights = readFloatArray(heightsV);
        if (heights.length <= 0) {
            throw new IllegalArgumentException("terrain.terrainHeights: heights is empty");
        }

        // size: allow cfg.size OR cfg.terrain.size OR infer from heights length
        int size = clampInt(num(cfg, "size", 0), 0, 8193);
        Value terr = member(cfg, "terrain");
        if (size <= 0 && terr != null && !terr.isNull()) {
            size = clampInt(num(terr, "size", 0), 0, 8193);
        }
        if (size <= 0) {
            int s = (int) Math.round(Math.sqrt(heights.length));
            if (s > 0 && s * s == heights.length) size = s;
        }
        if (size <= 1) {
        ...
        // patchSize: cfg.patchSize OR cfg.terrain.patchSize OR default
        int patchSize = clampInt(num(cfg, "patchSize", TerrainDefaults.PATCH_SIZE), 17, 257);
        if (terr != null && !terr.isNull() && !has(cfg, "patchSize")) {
        ...
        if (autoCenter) {
            ...
            if (min >= -0.0001f && max <= 1.0001f && (max - min) > 0.0001f) {
                for (int i = 0; i < heights.length; i++) heights[i] -= 0.5f;
            }
        }
