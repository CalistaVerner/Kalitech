...
function getHotReloadDomain(ctx) {
    if (!ctx) return null;
    try { if (typeof ctx.hotReload === "function") return ctx.hotReload(); } catch (_) {}
    try { if (ctx.hotReload && typeof ctx.hotReload.register === "function") return ctx.hotReload; } catch (_) {}
    return null;
}

function getStateDomain(ctx) {
    if (!ctx) return null;
    try { if (ctx.stateDomain && typeof ctx.stateDomain.get === "function") return ctx.stateDomain; } catch (_) {}
    try { if (typeof ctx.state === "function") return ctx.state(); } catch (_) {}
    try { if (ctx.state && typeof ctx.state.get === "function") return ctx.state; } catch (_) {}
    return null;
}
...
function ensureHotReloadHook(ctx) {
    ...
    hr.register(() => {
        try { module.exports.destroy(ctx); } catch (_) {}
    });
...
module.exports.destroy = function destroy(ctx) {
    void ctx;
    try { if (_player) _player.dispose(); } catch (_) {}
    _player = null;
};
